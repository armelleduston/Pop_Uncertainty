...
result_files <- list.files("full_sim/results", 
...
if (length(result_files) > 0) {
  ...
  summary_stats <- data.frame(
    Metric = c("NoBench_Bias", "NoBench_Coverage", "NoBench_Runtime",
               "Bench_Bias", "Bench_Coverage", "Bench_Runtime",
               "Diff_Bench_Pstar", "Diff_NoBench_Pstar", "Diff_Bench_NoBench"),
    Mean = c(
      mean(results_df$NoBench_Bias, na.rm = TRUE),
      mean(results_df$NoBench_Coverage, na.rm = TRUE),
      mean(results_df$NoBench_Runtime, na.rm = TRUE),
      mean(results_df$Bench_Bias, na.rm = TRUE),
    ...
    SD = c(
      sd(results_df$NoBench_Bias, na.rm = TRUE),
      sd(results_df$NoBench_Coverage, na.rm = TRUE),
      sd(results_df$NoBench_Runtime, na.rm = TRUE),
      sd(results_df$Bench_Bias, na.rm = TRUE),
      sd(results_df$Bench_Coverage, na.rm = TRUE),
      sd(results_df$Bench_Runtime, na.rm = TRUE),
      sd(results_df$Diff_Bench_Pstar, na.rm = TRUE),
      sd(results_df$Diff_NoBench_Pstar, na.rm = TRUE),
      sd(results_df$Diff_Bench_NoBench, na.rm = TRUE)
    )
  )
  print(summary_stats)
  
  cat("\nN_Failures:", sum(is.na(results_df$NoBench_Runtime)), "\n")
