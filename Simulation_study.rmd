...
### Write disctete Normal nimble dist

```{r ddnorm_nimble_function}
...
### Run model with realistic data ( benchmark)
...
print("RMSE between P and P*")
rmserr(true_Ps, obs_Ps)$rmse 
print("RMSE between P and no bench posterior P")
rmserr(true_Ps, apply(X=modeled_Ps_no_bench2, MARGIN=2 , FUN=median))$rmse
print("RMSE between P and inexact bench posterior P")
...
# Large Scale Simulation Results (from cluster)

```{r}
# Combine results from all simulation tasks

# Find all task-specific result files (new combined format)
result_files <- list.files("full_sim/results/sim_R500_Pbig", 
                           pattern = "^simulation_results_task_\\d+\\.csv$", 
                           full.names = TRUE)

cat(paste("Found", length(result_files), "task result files\n"))

# Combine all results into one dataframe
result_list <- lapply(result_files, read.csv)
results_df <- do.call(rbind, result_list)
results_df <- results_df[order(results_df$Run), ]
cat(paste("Combined results:", nrow(results_df), "runs\n"))
...
big_res <- data.frame(cbind(rbind(mean(results_df$NoBench_Bias, na.rm = TRUE),
                      mean(results_df$NoBench_Coverage, na.rm = TRUE),
                      mean(results_df$NoBench_Runtime, na.rm = TRUE),
                      mean(results_df$NoBench_Gelman_rho, na.rm = TRUE),
                      mean(results_df$NoBench_Gelman_log_kappa, na.rm = TRUE),
                      mean(results_df$NoBench_Gelman_S1, na.rm = TRUE),
                      mean(results_df$NoBench_Gelman_P1, na.rm = TRUE),
                      mean(results_df$Diff_Bench_Pstar, na.rm = TRUE),
                      mean(results_df$Diff_NoBench_Pstar, na.rm = TRUE),
                      mean(results_df$Diff_Bench_NoBench, na.rm = TRUE)),
                      rbind(mean(results_df$Bench_Bias, na.rm = TRUE),
                      mean(results_df$Bench_Coverage, na.rm = TRUE),
                      mean(results_df$Bench_Runtime, na.rm = TRUE),
                      mean(results_df$Bench_Gelman_rho, na.rm = TRUE),
                      mean(results_df$Bench_Gelman_log_kappa, na.rm = TRUE),
                      mean(results_df$Bench_Gelman_S1, na.rm = TRUE),
                      mean(results_df$Bench_Gelman_P1, na.rm = TRUE),
                      NA,
                      NA,
                      NA)))
colnames(big_res) <- c("No Bench", "Bench")
rownames(big_res) <- c("Bias", "Coverage",
             "Runtime", "Gelman_rho",
             "Gelman_log_kappa", "Gelman_S1",
             "Gelman_P1", "P_b - P*", 
             "P_nb - P*", "P_b - P_nb")

big_res |>
  kbl(caption = "Simulation Results (R=500)",
      booktabs = TRUE,
      digits = 5) |>
